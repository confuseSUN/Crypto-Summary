...
impl SimplifiedSWUMap<ark_secp256k1::Config> for Secp256K1SSWUMap {
    ...
    fn isogeny_map(x: &Fq, y: &Fq) -> Affine {
        let x_2 = x.square();
        let x_3 = x_2.mul(x);

        let x_num = x_3.mul(&K13).add(x_2.mul(K12)).add(x.mul(&K11)).add(&K10);
        let x_den = x_2.add(x.mul(&K21)).add(&K20);

        let y_num = x_3.mul(&K33).add(x_2.mul(&K32)).add(x.mul(&K31)).add(&K30);
        let y_den = x_3.add(x_2.mul(&K42)).add(x.mul(&K41)).add(&K40);

        let x = x_num.div(&x_den);
        let y = y_num.mul(y).div(&y_den);

        #[cfg(feature = "debug")]
        {
            use ark_ec::short_weierstrass::SWCurveConfig;
            let y_squared = (x * x * x).add(&ark_secp256k1::Config::COEFF_B);
            assert_eq!(y.square(), y_squared);
        }
